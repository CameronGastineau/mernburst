...
import { scaleLinear as d3ScaleLinear} from 'd3-scale'; //, scaleSqrt as d3ScaleSqrt } from 'd3-scale';
...
class Sunburst extends React.Component {
    ...
    constructor(props) {
        ...
        //this.y = d3ScaleSqrt()
            .range([0, this.radius]); //changes how the graph grows or shrinks to meet scaled size

        this.x= d3ScaleLinear()
            .range([0, 2 * Math.PI])

        this.arc = d3Arc()
            .startAngle((d) => { return Math.max(0, Math.min(2 * Math.PI, this.x(d.x0)+.003)); })
            .endAngle((d) => { return Math.max(0, Math.min(2 * Math.PI, this.x(d.x1))); })
            .innerRadius((d) => { return Math.max(0, this.y(d.y0)+1); }) //changes inner radius
    ...
    _middleArcLine(d) {
        ...
        const r = Math.max(0, (this.y(d.y0) + this.y(d.y1)) / 2); //TODO: Adjusts locations of labels
    ...
    _colorize(d) {
        ...
        //see if it even DESERVES color...
        const children = d.children ? d.children.length : 0
        const currentValue = (current.value ? current.value : 0)
        const avgValue = d.children ? currentValue/children : currentValue

        const gray = '#969696'

        //if it doesn't, return gray
        if (avgValue <= 1)
            return gray
